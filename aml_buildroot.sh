...
BUILD_OPTION_GPU_CONFIG="VIP9000NANODI_PID0XE8"
...
echo make ARCH=$ARCH CC=$CC HOSTCC=$HOSTCC LD=$LD NM=$NM OBJCOPY=$OBJCOPY M=$M O=$O CLANG_TRIPLE=$CROSS_COMPILE CROSS_COMPILE=$CROSS_COMPILE -f makefile.linux $BUILD_OPTIONS gal_core V_TARGET=all
make ARCH=$ARCH CC=$CC HOSTCC=$HOSTCC LD=$LD NM=$NM OBJCOPY=$OBJCOPY M=$M O=$O CLANG_TRIPLE=$CROSS_COMPILE CROSS_COMPILE=$CROSS_COMPILE -f makefile.linux $BUILD_OPTIONS gal_core V_TARGET=all
#make ARCH=$ARCH CC=$CC HOSTCC=$HOSTCC LD=$LD NM=$NM OBJCOPY=$OBJCOPY M=$M O=$O CLANG_TRIPLE=aarch64-linux-gnu- CROSS_COMPILE=aarch64-linux-gnu- -f makefile.linux $BUILD_OPTIONS gal_core V_TARGET=all
#unset KERNEL_DIR
