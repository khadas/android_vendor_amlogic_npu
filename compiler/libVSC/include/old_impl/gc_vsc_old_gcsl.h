...
typedef enum _gcSHADER_FLAGS
{
    ...
    gcSHADER_FLAG_HAS_32BITMODULUS          = 0x400, /* the shader has 32 bit modulus and has been converted to 16 bit ops.  */
    gcSHADER_FLAG_HAS_BASE_MEMORY_ADDR      = 0x800, /* the shader has base memory address, for CL only. */
    gcSHADER_FLAG_HAS_LOCAL_MEMORY_ADDR     = 0x1000, /* the shader has local memory address. */
    gcSHADER_FLAG_HAS_INT64                 = 0x2000, /* the shader has 64 bit integer data and operation. */
    gcSHADER_FLAG_HAS_IMAGE_QUERY           = 0x4000, /* the shader has image query */
    gcSHADER_FLAG_HAS_VIV_VX_EXTENSION      = 0x8000, /* the shader has Vivante VX extension */
    gcSHADER_FLAG_USE_LOCAL_MEM             = 0x10000, /* the shader use local memory */
    gcSHADER_FLAG_VP_TWO_SIDE_ENABLE        = 0x20000, /* the shader use two side, for GL fragment shader only. */
    gcSHADER_FLAG_CLAMP_OUTPUT_COLOR        = 0x40000, /* Clamp the output color, for GL shader only. */
    gcSHADER_FLAG_HAS_INT64_PATCH           = 0x80000, /* the shader has 64 bit integer data and operation patch (recompile). */
    gcSHADER_FLAG_AFTER_LINK                = 0x100000, /* the shader is linked(gcLinkProgram/gcLinkShaders/gcLinkKernel). */
    gcSHADER_FLAG_LOADED_KERNEL             = 0x200000, /* shader has loaded a specified kernel function, for OCL shader only. */
